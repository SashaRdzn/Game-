  ...
  return (
    <main className="desktop" onClick={() => startOpen && setStartOpen(false)}>
      ...
      {zOrder.map((app, index) => windows[app] === "open" && <Window key={app} title={windowTitle(app)} active={zOrder[zOrder.length - 1] === app} layer={index} app={app} onFocus={() => focusWindow(app)} onClose={() => closeWindow(app)} onMinimize={() => minimizeWindow(app)}>
        ...
        {app === "archive" && <Archive restored={restored} code={archiveCode} setCode={setArchiveCode} unlock={unlockArchive} done={chapterDone} openLink={() => { if (browserInstalled) setLinkWarning("pika://oblako-foto/xxx-228-lox/"); else setProtocolError(true); }} openCryptoLink={() => { if (browserInstalled) setLinkWarning("pika://crypto-ne-naebalovo-100%/walet/526967148866"); else setProtocolError(true); }} openHackLink={() => { if (browserInstalled) setLinkWarning("pika://blackbox-market/device/universal-hack"); else setProtocolError(true); }} />}
...
function MalwareStorm({ onClose }: { onClose: () => void }) {
  const alerts = ["KERNEL ACCESS GRANTED", "UPLOADING MEMORY", "UNKNOWN_17 CONNECTED", "DO NOT CLOSE", "SYSTEM32 SOLD", "HACK DEVICE ACTIVE"];
  return <div className="malware-storm">{Array.from({ length: 28 }, (_, index) => <div key={index} className="malware-window" style={{ left: `${(index * 17) % 78}%`, top: `${(index * 29) % 72}%`, zIndex: 200 + index, transform: `rotate(${(index % 5) - 2}deg)` }}><header>UNIVERSAL HACK DEVICE <button onClick={onClose}>×</button></header><div><b>!</b><span>{alerts[index % alerts.length]}</span></div></div>)}<div className="malware-center"><h2>УДАЛЁННЫЙ ДОСТУП ПЕРЕХВАЧЕН</h2><p>Попытка закрыть любое окно приведёт к аварийному перезапуску.</p><button onClick={onClose}>ЗАКРЫТЬ ВСЁ</button></div></div>;
}
...
function Archive({ restored, code, setCode, unlock, done, openLink, openCryptoLink, openHackLink }: { restored: boolean; code: string; setCode: (v: string) => void; unlock: () => void; done: boolean; openLink: () => void; openCryptoLink: () => void; openHackLink: () => void }) {
  if (done) return <div className="archive-unpacked"><div className="archive-header"><div className="seal">I</div><div><p className="eyebrow">КОНТЕЙНЕР РАСШИФРОВАН</p><h2>НАСЛЕДСТВО.arc</h2></div></div><p>В архиве обнаружено пять объектов:</p><div className="archive-files"><button onDoubleClick={openLink} onClick={openLink}><span>◎</span><b>ENTRY_POINT.url</b><small>pika://oblako-foto/xxx-228-lox/</small></button><button onDoubleClick={openCryptoLink} onClick={openCryptoLink}><span>₿</span><b>WALLET_RECOVERY.url</b><small>pika://crypto-ne-naebalovo-100%/walet/526967148866</small></button><button onDoubleClick={openHackLink} onClick={openHackLink}><span>⚡</span><b>FREE_HACK_DEVICE.url</b><small>pika://blackbox-market/device/universal-hack</small></button><div><span>▤</span><b>browser_note.txt</b><small>«Обычные браузеры этого адреса не увидят»</small></div><div><span>◇</span><b>signature.key</b><small>PIKA LEGACY CERTIFICATE</small></div></div><div className="archive-tip">Откройте URL-файл, чтобы перейти на соответствующий PIKA-узел.</div></div>;

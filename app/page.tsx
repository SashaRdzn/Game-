...
const files: Record<FileId, { name: string; icon: string; app: AppId }> = {
  ...
  photo: { name: "IMG_0314.corrupt", icon: "▧", app: "files" },
...
export default function Home() {
  ...
  function runCommand() {
    ...
    if (cmd === "status") response = "Том RECOVERY: 63%  |  Последний вход: unknown_17  |  03:14";
    if (cmd === "cat system.log") response = "[03:14] login: unknown_17\n[03:17] deleted: user_fragment.log\n[03:18] archive locked";
    if (cmd === "cat user_fragment.log" || cmd === "cat .trash/user_fragment.log") response = restored ? "Восстановлено: KEY_HINT = FIRST_MEMORY" : "Ошибка: файл помечен как удалённый. Используйте recover.";
    if (cmd === "recover user_fragment.log" || cmd === "recover .trash/user_fragment.log") {
      response = "Восстановление завершено. KEY_HINT = FIRST_MEMORY";
      setRestored(true);
  ...
  function unlockArchive() {
    ...
    } else setNotice("Ключ отклонён. Подсказка: FIRST_MEMORY");
  }
  ...
  return (
    <main className="desktop" onClick={() => startOpen && setStartOpen(false)}>
      ...
      <section className="icons" aria-label="Рабочий стол">
        <DesktopIcon icon="▤" label="ПРОЧТИ_МЕНЯ.txt" onClick={() => setOpenApp("readme")} />
        <DesktopIcon icon="▣" label="НАСЛЕДСТВО.arc" onClick={() => setOpenApp("archive")} />
        <DesktopIcon icon="▧" label="Мои файлы" onClick={() => setOpenApp("files")} />
        <DesktopIcon icon="⌫" label="Корзина" onClick={() => { setSelectedFile("deleted"); setOpenApp("files"); }} />
        <DesktopIcon icon=">_" label="Терминал" onClick={() => setOpenApp("terminal")} />
      </section>

      {openApp && <Window title={windowTitle(openApp)} onClose={() => setOpenApp(null)}>
        ...
        {openApp === "log" && <pre className="text-file">[03:14:08] LOGIN unknown_17\n[03:17:42] DELETE user_fragment.log\n[03:18:01] LOCK inheritance.arc\n[03:18:07] SESSION LOST</pre>}
      ...
      <footer className="taskbar">
        ...
        {openApp && <button className="task-button" onClick={() => setOpenApp(openApp)}>{windowTitle(openApp)}</button>}
...
function Window({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <section className="window"><header className="titlebar"><span>▥ {title}</span><div><button aria-label="Свернуть">_</button><button aria-label="Закрыть" onClick={onClose}>×</button></div></header><nav className="menubar">Файл　 Правка　 Вид　 Справка</nav><div className="window-content">{children}</div><div className="window-status">MEMORIA REMOTE VOLUME · READ ONLY</div></section>; }
...
function Readme({ onContinue }: { onContinue: () => void }) { return <article className="document letter"><div className="stamp">URGENT<br/><b>14 MAR</b></div><p className="mono-meta">RECOVERY NOTE / НЕ ОТПРАВЛЯТЬ ПО ПОЧТЕ</p><h1>Если ты это читаешь — резервный канал сработал.</h1><p>Кто-то получил доступ к моему компьютеру. Я успел перенести сюда несколько файлов, связанных с наследством, но часть данных удалена.</p><p>Не верь времени на экране. Смотри на то, что система пытается скрыть. Начни с повреждённой фотографии и журнала входов.</p><p>Пароль от архива я не мог записать прямо. Подсказка была нашей <b>первой памятью</b>.</p><p className="signature">— твой брат<br/><small>P.S. Если увидишь пользователя <b>unknown_17</b>, отключись.</small></p><button className="system-button" onClick={onContinue}>Открыть файлы →</button></article>; }
...
function FilePreview({ id, restored, openArchive }: { id: FileId; restored: boolean; openArchive: () => void }) {
  if (id === "photo") return <div className="preview"><div className="corrupt-photo"><span>03:14</span><i>DATA LOST</i></div><p><b>IMG_0314.corrupt</b></p><p>Дата создания повреждена. В уцелевшем фрагменте часов: <b>03:14</b>.</p></div>;
  if (id === "note") return <div className="preview"><p><b>заметка_без_даты.txt</b></p><p>«Первое, что машина запомнила, — не лицо и не имя. Только время».</p></div>;
  if (id === "log") return <div className="preview"><p><b>system.log</b></p><pre>[03:14] login: unknown_17{`\n`}[03:17] delete: user_fragment.log</pre></div>;
  if (id === "deleted") return <div className="preview"><p><b>user_fragment.log</b> · восстановлен</p><pre>KEY_HINT = FIRST_MEMORY{`\n`}ARCHIVE ATTEMPTS = 2</pre></div>;
